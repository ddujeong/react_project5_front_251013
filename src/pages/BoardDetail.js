...
const BoardDetail = ({user}) => {
    ...
    const handleDelete =  async() => {
        ...
        const loadComments = async() => { // 특정 글 id로 댓글 목록 요청하기
        try{
            setLoading(true);
        ...
        const handleCommentUpdate = async(commentId) => {
            ...
            try {
                const res = await api.put(`/api/comment/${commentId}`, {content : editingCommentContent});
                alert("댓글 수정 성공!");
                    ...
                    {comments.map((c)=> (
                        <li key={c.id} className='comment_item'>
                            <div className='comment_header'>
                                <span className='comment_author'>작성자 : {c.author.username}</span>
                                <span className='comment_date'>{formatDate(c.createdate)}</span> 
                            </div>
                            {editingCommentId === c.id ? (
                                <>
                                    {/* 댓글 수정 섹션 시작 */}
                                    <textarea value={editingCommentContent} onChange={(e) => setEditingCommentContent(e.target.value)}></textarea>
                                    <button className='comment_save' onClick={() => handleCommentUpdate(c.id)}>저장</button>
                                    <button className='comment_cancle' onClick={() => setEditingCommentId(null)}>취소</button>
                                    {/* 댓글 수정 섹션 끌 */}
                                </>
                            ):(
                                <>                                
